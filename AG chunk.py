...
INPUT_PATH = r"D:\KI-Agent\data\spo_allgemein\allgemeinerteil_bachelor_clean.jsonl"
OUTPUT_PATH = r"D:\KI-Agent\out\AG chunk.jsonl"
DOC_SHORT_NAME = "AG_SPO"  # 文档短名 / Kurzname des Dokuments
...
def guess_section(full_text: str, page: int) -> str:
    """
    粗略猜测这一页属于哪个 section。
    Allgemeiner Teil 里可能也有 § 编号，这里先用一个通用版：
    - 如果这一页出现 '§'，就标记 'Seite X (mit §)'；
    - 否则就叫 'Seite X'。

    Grobe Schätzung, zu welchem Abschnitt die Seite gehört.
    Im Allgemeinen Teil gibt es evtl. auch §§:
    - Wenn die Seite ein '§' enthält -> 'Seite X (mit §)';
    - sonst einfach 'Seite X'.
    """
    if "§" in full_text:
        return f"Seite {page} (mit §)"
    return f"Seite {page}"

# 这个函数只是给每一页打个标签，方便以后大致知道这一页属于哪里。
# Diese Funktion vergibt einen groben Abschnitts-Namen für jede Seite,
# damit man später ungefähr weiß, wo der Chunk herkommt.
...
def main():
    # 0. 先检查输入文件在不在 / Eingabedatei prüfen
    ...
    if not pages:
        print("❌ Es wurden keine Seiten geladen. Bitte prüfe Inhalt/Format der Datei.")
        return
